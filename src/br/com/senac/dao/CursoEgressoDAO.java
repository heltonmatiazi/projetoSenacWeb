...
public class CursoEgressoDAO implements Serializable {
    ...
    public void inserir(CursoEgresso curso) throws SQLException {
        ...
        pstm.setInt(2, curso.getCurso().getIdCurso());
    ...
    public boolean alterar(CursoEgresso curso) throws SQLException {
        ...
        conn = Conexao.getConnection();
        pstm = conn.prepareStatement(
                "UPDATE CURSOEGRESSO SET IDCURSO =?, UNIDADESENAC = ?, ANOINGRESSO = ?, SEMESTREINGRESSO = ?, ANOCONCLUSAO = ?, SEMESTRECONCLUSAO = ?, IDTITULO = ? WHERE IDCURSOEGRESSO = ?");
        pstm.setInt(1, curso.getCurso().getIdCurso());
